...
### ---------------------------- Step1: 准备count数据 ----------------------------------------------------------- ###
# (1) 读取count数据
count_file <- "prepare_counts/featureCounts_counts_filter1.csv"
counts_filtered <- read_csv(count_file,locale = locale(encoding = "UTF-8"))
counts_matrax <- counts_filtered[, 3:6] 
...
# (4) 保存差异分析结果
write.csv(res_ged_counts , file = "DESeq2_results/featureCount_DESeq2_res_ged_counts.csv", row.names = FALSE)

### ————————————————————————————— Step5: 注释gene_name, TPM  ——————————————————————————————---------------------------------———-—————— ###

# (1) 读取注释数据
# gene_id, gene_name, transcript_id
anno_file <- "prepare_counts/featureCounts_gene_TPM_geneName_filter1.csv"
...
write.csv(res_ged_counts_geneName_TPM_Allgene, 
          file = "DESeq2_results/featureCount_DESeq2_res_ged_counts_geneName_TPM_Allgene.csv",
          row.names = FALSE)
...
write.csv(as.data.frame(res_ged_counts_geneName_TPM_Siggene), 
          file = "DESeq2_results/featureCount_DESeq2_res_ged_counts_geneName_TPM_Siggene.csv",
          row.names = FALSE)
