...
print(paste("所有样本中counts > 0的基因数:", sum(rowSums(counts_data[, 3:6]) > 0)))
...
print(paste("所有样本中tpm > 0的基因数:", sum(rowSums(TPM_results[, 2:5]) > 0)))
...
# (1) 读取gene_id_name mapping table
...
print(paste(nrow(TPM_results),"个gene注释了", sum(success_anno), "个gene_name"))
...
keep_feature <- rowSums(counts_data[, 3:6]>=1) >= 1 # rowSums()对每行（每个基因）的TRUE值求和
...
dat_with_id$gene_id <- rownames(dat)  # 如果行名是基因ID
