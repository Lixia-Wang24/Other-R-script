...
edgeR_res_file <-   "HTSeqcount_labE/edgeR_results/differential_expression_results.csv"

HTSeqcount_dif <- as.data.frame(read_csv(HTSeqcount_dif_file, col_names = TRUE, locale = locale(encoding = "UTF-8")))
edgeR_res_dif <- as.data.frame(read_csv(edgeR_res_file, col_names = TRUE, locale = locale(encoding = "UTF-8")))

print(paste("DESeq2检测到的差异表达基因数量为:", nrow(HTSeqcount_dif)))
print(paste("edgeR检测到的差异表达基因数量为:", nrow(edgeR_res_dif)))
...
edgeR <- edgeR_res_dif %>% 
  select(gene_id, log2FC_edgeR = logFC)

# 创建总表（全外连接）
combined <- full_join(DESeq2, edgeR, by = "gene_id") %>%
  replace(is.na(.), 0)  # 将NA替换为0


### ————————————————————————————— Step7: sig_gene log2FC 的scaterplot (3种方法两两之间取交集)———————————————---------------------——-—————---------— ###

DESeq2_edgeR_inter <- combined %>%
  filter((log2FC_DESeq2 != 0 & log2FC_edgeR != 0))

DESeq2_edgeR_inter_pearson <- cor(DESeq2_edgeR_inter[, 2:3], method = "pearson")

DESeq2_edgeR_inter_spearman <- cor(DESeq2_edgeR_inter[, 2:3], method = "spearman")
...
# 生成并保存散点图
DESeq2_edgeR_inter_cor <- create_scatter(DESeq2_edgeR_inter, "log2FC_DESeq2", "log2FC_edgeR")

# 保存图形（PNG格式）
ggsave("3counts_camperation/DESeq2_edgeR_inter_cor_logFC_scaterplot.png", DESeq2_edgeR_inter_cor, width = 8, height = 6, dpi = 300)

# 结果解释
# 图中右上角出现两条近似平行的黑点线，是由于 低表达基因在计算 log2FC 时引入伪计数（pseudocount）导致的离散化效应。
# DESeq2 默认使用 log2(n + 1) 的伪计数策略（n 为原始计数）。
# edgeR 默认使用 log2(n + 0.125) 的伪计数（通过 prior.count 参数调整）。
...
DESeq2_edgeR_union <- combined %>%
  filter(!(log2FC_DESeq2 == 0 & log2FC_edgeR == 0))

DESeq2_edgeR_union_pearson <- cor(DESeq2_edgeR_union[, 2:3], method = "pearson")

DESeq2_edgeR_union_spearman <- cor(DESeq2_edgeR_union[, 2:3], method = "spearman")
...
# 生成并保存散点图
DESeq2_edgeR_union_cor <- create_scatter(DESeq2_edgeR_union, "log2FC_DESeq2", "log2FC_edgeR")

# 保存图形（PNG格式）
ggsave("3counts_camperation/DESeq2_edgeR_union_cor_logFC_scaterplot.png", DESeq2_edgeR_union_cor, width = 8, height = 6, dpi = 300)
